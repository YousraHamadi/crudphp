...
<header>
    ...
    <?php
    if(count($_POST)>0) {
        mysqli_query($conn,"UPDATE livres set titre= $titre,
        auteur=$auteur,
        images=$images, 
         prix=$prix,
        quantite=$quantite,
        date_achat=$date_achat,
        date_de_publication=$date_de_publication  WHERE id='".$_POST['id']."'");
        }
    $result = mysqli_query($conn,"SELECT * FROM livres WHERE id='".$_GET["id"]."'");
